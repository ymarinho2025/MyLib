...
class AuthController {
    public static function register(): void {
        ...
        $password = $d['password'] ?? '';
        $bio = trim($d['bio'] ?? '');
        if (!$name || !$username || !$email || strlen($password) < 6) json_response(['error'=>'Dados inválidos. Senha mínima: 6 caracteres.'],422);
        if (!preg_match('/^[a-z0-9._]+$/', $username)) json_response(['error'=>'Username aceita apenas letras minúsculas, números, ponto e underline.'],422);
        ...
        try {
            ...
            if ($e->getCode() === '23505') json_response(['error'=>'E-mail ou username já cadastrado.'],409);
            throw $e;
        }
    }
    public static function login(): void {
        ...
        $user = $stmt->fetch();
        if (!$user || !password_verify($password, $user['password_hash'])) json_response(['error'=>'Login ou senha inválidos.'],401);
